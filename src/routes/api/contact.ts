 ...
 *
 * Sends the enquiry to the studio through Resend's REST API. Until
 * RESEND_API_KEY and CONTACT_FROM_EMAIL are set it replies 501, and the form
 * falls back to a prefilled WhatsApp message.
 */
...
async function handlePost({ request }: { request: Request }) {
  ...
  if (!apiKey || !from) return json({ ok: false, reason: "not-configured" }, 501);
